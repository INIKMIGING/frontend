...
import { Search, Grid3x3, List, Thermometer, Download, ChevronDown, Loader2 } from 'lucide-react';
...
export function Temperature() {
  ...
  const { metrics, loading } = useTemperatureMetrics(
  ...
  // =========================================================================
  // EXPORT EXCEL (Tanpa Max Threshold)
  // =========================================================================
  const handleExportExcel = async (range: string) => {
    ...
    try {
      ...
      const rawBackendData = payload['temperature'] || []; 

      let formattedData: any[] = [];

      if (rawBackendData.length > 0) {
        console.log(`Menarik ${rawBackendData.length} baris suhu. Melakukan merging...`);
        ...
        formattedData = filteredMetrics.map(m => {
          ...
          if (serverItems.length > 0) {
            ...
            finalAvg = m.status === 'unknown' ? 'N/A' : (m.temperature !== null ? Number(m.temperature.toFixed(1)) : 'N/A');
          }
          ...
          return {
            ...
            "Max Temp (°C)": finalMax,
            "Avg Temp (°C)": finalAvg
            // Max Threshold dihapus
          };
        });
      } else {
        console.warn(`Data server kosong. Menggunakan data tabel UI fallback.`);
        formattedData = filteredMetrics.map(m => ({
          "Device Name": m.deviceName,
          "IP Address": m.ipAddress || 'Unknown',
          "Location": m.location || 'Unknown',
          "Min Temp (°C)": "N/A",
          "Max Temp (°C)": "N/A",
          "Avg Temp (°C)": m.status === 'unknown' ? 'N/A' : (m.temperature !== null ? Number(m.temperature.toFixed(1)) : 'N/A')
          // Max Threshold dihapus
        }));
      }
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, `Temperature_${range}`);
      const dateStr = new Date().toISOString().split('T')[0];
      XLSX.writeFile(workbook, `Report_Temperature_${range}_${dateStr}.xlsx`);
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-zinc-500">Loading temperature metrics...</p>
        </div>
      </div>
    );
  }
  ...
  const avgTemp = validMetricsForAvg.reduce((sum, m) => sum + m.temperature, 0) / (validDevicesCount || 1);
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      case 'unknown': return { color: 'bg-slate-400 dark:bg-zinc-600', borderColor: 'border-slate-300 dark:border-zinc-700', bgColor: 'bg-slate-100 dark:bg-zinc-800', textColor: 'text-slate-500 dark:text-zinc-500' };
  ...
  return (
    <>
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-zinc-100 mb-2">Temperature Monitoring</h1>
        <p className="text-slate-600 dark:text-zinc-500">Device temperature tracking and thermal alerts (Inlet Sensors)</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 rounded-lg p-4 shadow-sm flex flex-col justify-center">
          <div className="flex justify-between items-start">
            <p className="text-sm text-slate-600 dark:text-zinc-500 mb-1">Avg Temp</p>
          ...
          <p className="text-2xl font-bold text-slate-900 dark:text-zinc-100 mt-1">
            {avgTemp > 0 ? avgTemp.toFixed(1) : 0}°C
          </p>
        </div>
      </div>
      ...
      {filteredMetrics.length === 0 ? (
        <div className="bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 rounded-lg p-12 text-center shadow-sm">
          <p className="text-slate-600 dark:text-zinc-500">No temperature metrics found matching your search.</p>
        </div>
      ) : viewMode === 'table' ? (
            ...
            <table className="w-full">
              <thead className="bg-slate-100 dark:bg-zinc-800 border-b border-slate-300 dark:border-zinc-700">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-700 dark:text-zinc-400 uppercase tracking-wider">Current Temp</th>
                  {/* Kolom Header Max Threshold Dihapus */}
                    ...
                    <tr key={metric.deviceId} className="hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <Thermometer className={`w-4 h-4 ${config.textColor}`} />
                          <span className="text-sm text-slate-800 dark:text-zinc-300 font-medium">
                            {metric.status === 'unknown' ? 'N/A' : `${metric.temperature.toFixed(1)}°C`}
                          </span>
                        </div>
                      </td>
                      {/* Sel Data Max Threshold Dihapus */}
                ...
                <div className="space-y-3 mb-4">
                  ...
                  <div className="bg-slate-100 dark:bg-zinc-800 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <Thermometer className={`w-5 h-5 ${config.textColor}`} />
                      <span className={`text-2xl font-bold ${config.textColor}`}>
                        {metric.status === 'unknown' ? 'N/A' : `${metric.temperature.toFixed(1)}°C`}
                      </span>
                    </div>
                    <div className="bg-slate-200 dark:bg-zinc-700 rounded-full h-2">
                      <div className={`h-2 rounded-full ${config.color}`} style={{ width: metric.status === 'unknown' ? '0%' : `${Math.min((metric.temperature / 75) * 100, 100)}%` }} />
                    </div>
                    <p className="text-xs text-slate-600 dark:text-zinc-500 mt-2">Current Temperature</p>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setSelectedDevice(null)}>
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div className="bg-zinc-800 rounded-lg p-4">
                <p className="text-xs text-zinc-500 mb-1">Current Temp</p>
                <p className="text-lg font-bold text-zinc-100">
                  {activeDeviceData.status === 'unknown' ? 'N/A' : `${activeDeviceData.temperature.toFixed(1)}°C`}
                </p>
              </div>
            </div>
            ...
            <div className="bg-zinc-800 rounded-lg p-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-semibold text-zinc-100">
                    Temperature History ({isCustomRange && selectedDate ? 'Selected Date' : selectedPreset === '1h' ? 'Last 1h' : selectedPreset === '6h' ? 'Last 6h' : 'Last 24h'})
                  </h3>
                </div>
              ...
              <LineChart data={activeDeviceData.history} dataKey="temperature" xAxisKey="timestamp" color="#facc15" height={300} />
